#!/bin/bash
# One-time setup of the tamago-go toolchain for the racccoon Go port:
# clone the version-matched branch, apply lib/go/racccoon.patch (the os
# <-> fsd bridge hooks), and build it. Prints the TAMAGO export line to
# use with scripts/build_go.sh afterward. See docs/go-port-plan.md.
#
#   bash scripts/setup_tamago.sh [dest-dir]   (default: ./third_party/tamago-go)
set -e
...
if git apply --reverse --check "$PATCH" 2>/dev/null; then
  ...
  echo "setup_tamago.sh: lib/go/racccoon.patch does not apply cleanly to $DEST" >&2
  echo "  (branch mismatch? this patch is for $BRANCH)" >&2
  exit 1
fi
